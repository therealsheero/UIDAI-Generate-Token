...
CREATE TABLE physical_reference_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, age INTEGER, mobile TEXT, aadhaar_last4 TEXT, gender TEXT, district TEXT, service_type TEXT, token_type TEXT, created_at TEXT DEFAULT (datetime('now','localtime')));

ALTER TABLE physical_reference_entries ADD COLUMN token TEXT;

ALTER TABLE physical_reference_entrie ADD COLUMN reference_of TEXT;

UPDATE physical_reference_entries SET token = CASE WHEN token LIKE '%-R-%' THEN REPLACE(token, '-R-', '-RT-') WHEN token LIKE '%-P-%' THEN REPLACE(token, '-P-', '-PT-') ELSE token END;

UPDATE daily_slots SET appointment_total = 140, walkin_total = 70 WHERE date > '2026-05-08' AND strftime('%w', date) != '1';