...
async function undoEntry(tokenId) {
  await fetch("http://localhost:5000/api/guard/undo-entry", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-guard-token": guardToken
...
async function loadTokens() {
    const tphRes = await fetch("http://localhost:5000/api/guard/tokens-per-hour", {
  ...
  const res = await fetch("http://localhost:5000/api/guard/entry-tokens", {
    headers: { "x-guard-token": guardToken }
  });
